"""
Workers maintain persistent HTTP connections with "keep-alive" to Proxmox VE.
Under the hood, proxmoxer uses the requests library (which supports keep-alive out-of-the-box).
By reusing a thread-local ProxmoxAPI instance per worker thread, connections are reused
efficiently across all requests dispatched to the thread pool.
"""
